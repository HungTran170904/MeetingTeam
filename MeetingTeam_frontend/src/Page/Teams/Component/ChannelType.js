...
const ChannelTypeItem=({team, type, setChannelInfo, channelInfo})=>{
          ...
          function handleClickChannel(teamId, channelIndex){
                    setChannelInfo({teamIndex: teams.findIndex(team=>team.id==teamId), channelIndex: channelIndex});
          }
          return(
                    <div className="accordion-item">
                              <h2 className="accordion-header">
                              <button className="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target={"#"+type+team.id} aria-expanded="false" aria-controls={type+team.id}>
                                       # {type}
                              </button>
                              </h2>
                              ...
                              <div className="accordion-body">
                                        {team.channels.map((channel,index)=>{
                                                  if(channel.type==type) return(
                                                            <a className={"list-group-item list-group-item-action"+(index===channelInfo.channelIndex?" h6":"")} onClick={()=>handleClickChannel(team.id,index)} key={index}>
                                                            # {channel.channelName}
                                                            </a>
                                                  )
                                        })}
